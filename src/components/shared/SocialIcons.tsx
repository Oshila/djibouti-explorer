...
function SocialIcon({ icon, href, label }: SocialIconProps) {
  return (
    <a
      ...
      aria-label={label}
      className="text-cream/40 hover:text-ochre transition-colors"
    >
      {icon}
    </a>
  );
...
export function SocialIcons({ locale }: Props) {
  // Import SVGs from simple-icons
  // Note: We're using dynamic imports to avoid SSR issues with SVGs
  const socialLinks = {
    facebook: {
  ...
  return (
    <div className="flex gap-4">
      ...
      <SocialIcon
        ...
        label={socialLinks.youtube.label}
        icon={
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
