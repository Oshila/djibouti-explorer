...
export async function GET() {
  try {
    // Check if destinations already exist using Admin SDK
    const snapshot = await adminDb.collection('destinations').limit(1).get();
    
    if (!snapshot.empty) {
      return NextResponse.json({
        success: true,
        message: 'Destinations already exist in Firestore. No action needed.',
        alreadySeeded: true
      });
    }

    // Add all destinations using Admin SDK
    let count = 0;
    for (const dest of destinations) {
      await adminDb.collection('destinations').doc(dest.id).set(dest);
      count++;
    }
